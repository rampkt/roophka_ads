...
class ads
{
	...
	public function getAllads($date) {
		...
		$query = 'SELECT ad.id,ad.userid,ad.adid,ad.detail,ad.type,ad.date_added FROM roo_transaction AS ad WHERE ad.date_added like '."'%$datestr%'".' and ad.type='."'add'".' AND adid > 0 order by ad.date_added desc LIMIT '.$this->start.','.$this->rowLimit;
