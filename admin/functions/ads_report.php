...
class ads
{
	...
	public function getAllads($date) {
		...
		$query = 'SELECT ad.id,ad.userid,ad.adid,ad.detail,ad.type,ad.date_added,ad.demo,ad.visitor_location,ad.ipaddr,(SELECT ra.type FROM roo_ads AS ra WHERE ra.id = ad.adid) AS adtype FROM roo_transaction AS ad WHERE ad.date_added like '."'%$datestr%'".' and ad.type='."'add'".' AND adid > 0 order by ad.date_added desc LIMIT '.$this->start.','.$this->rowLimit;
