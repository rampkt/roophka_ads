...
if(isset($_REQUEST['action']) AND isset($_REQUEST['id']) AND $_REQUEST['id'] > 0) {
	...
	if($_REQUEST['action'] == 'activate') {
		$plans->Activatecategory($_REQUEST['id']);
		redirect(HTTP_PATH . "admin/plancategory.php?category=$category&success=1");
	}
	if($_REQUEST['action'] == 'deactivate') {
		$plans->Deactivatecategory($_REQUEST['id']);
		redirect(HTTP_PATH . "admin/plancategory.php?category=$category&success=2");
	}
	if($_REQUEST['action'] == 'delete') {
		$plans->Deletecategory($_REQUEST['id']);
		redirect(HTTP_PATH . "admin/plancategory.php?success=4");
	}


	redirect(HTTP_PATH . "admin/plancategory.php");
}

if(isset($_REQUEST['action'])and ($_REQUEST['action']=="_addcategory")) {
	$plans->categoryname=$_REQUEST['name'];
	
		$plans->categorysave();
		redirect(HTTP_PATH . "admin/plancategory.php?success=3");
...
list($categoryList,$pagination) = $plans->getAllplancategory($category);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
			...
			<div class="row-fluid ">	
				<div class="box span12">
					...
					<div class="box-header">
						...
						<div class="box-icon">
						<form name="search" action="plancategory.php" method="get">
						...
						<table class="table table-bordered table-striped table-condensed">
							  ...
							  ?>
								<tr>
                                    ...
                                    <td>
                                    	<? if($ctgry['status'] == 0) { ?>
                                        <a href="./plancategory.php?action=deactivate&id=<?=$ctgry['id']?>&category=<?php echo $category2;?>" onClick="return confirm('Do you really want to deactivate this account?');" class="btn btn-small"><i class="halflings-icon white remove"></i>Deactiavte</a>
                                        <? } elseif($ctgry['status'] == 1) { ?>
                                        <a href="./plancategory.php?action=activate&id=<?=$ctgry['id']?>&category=<?php echo $category2;?>" onClick="return confirm('Do you really want to activate this account?');" class="btn btn-small btn-success"><i class="halflings-icon white ok">&nbsp;</i>Actiavte</a>
                                        <? } ?>
                                        
										<a href="./plancategory.php?action=delete&id=<?=$ctgry['id']?>" onClick="return confirm('Do you really want to delete this account?');" class="btn btn-small btn-primary"><i class="halflings-icon white remove">&nbsp;</i>Delete</a>
    ...
    <? include('./includes/footer.php'); ?>
		...
		<div class="modal-body" id="categoryResult">
			...
			<form action="plancategory.php" method="post" name="addcategory">
