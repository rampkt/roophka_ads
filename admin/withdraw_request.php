...
if(isset($_REQUEST['action']) AND isset($_REQUEST['id']) AND $_REQUEST['id'] > 0) {
	if($_REQUEST['action'] == 'activate') {
		$withdraw->Activate($_REQUEST['id']);
		redirect(HTTP_PATH . "admin/withdraw_request.php?success=2");
	}
	if($_REQUEST['action'] == 'deactivate') {
		$withdraw->Deactivate($_REQUEST['id']);
		redirect(HTTP_PATH . "admin/withdraw_request.php?success=2");
	}
	if($_REQUEST['action'] == 'demo') {
		$withdraw->DemoAccount($_REQUEST['id']);
		redirect(HTTP_PATH . "admin/withdraw_request.php?success=2");
	}
	redirect(HTTP_PATH . "admin/withdraw_request.php");
}
						...
						<table class="table table-bordered table-striped table-condensed">
							  ...
							  ?>
								<tr>
                                    ...
                                    <td>
									<a href="./withdraw_approve.php?action=activate&id=<?=$wdrw['id']?>" onClick="return confirm('Do you realy want to activate this account?');" class="btn btn-small btn-success" style="padding:0px 10px;"><i class="halflings-icon white ok">&nbsp;</i>Approve</a>
                                    	
                                        <a href="./withdraw_decline.php?action=deactivate&id=<?=$wdrw['id']?>" onClick="return confirm('Do you realy want to deactivate this account?');" class="btn btn-small" style="padding:0px 10px;"><i class="halflings-icon white remove"></i>Decline</a>
